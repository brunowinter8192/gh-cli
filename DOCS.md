...
CLI tool delivering 11 GitHub research commands (search, browse, issues, discussions, releases) plus RAG indexing. `cli.py` is the argparse entry point; each subcommand delegates to a `<tool>_workflow()` function in `src/github/`. Delivered to Claude Code sessions via the `gh-cli` wrapper and `github-search` skill — invoked through Bash calls, no MCP protocol.

## Entry Points

- `cli.py` → 11 argparse subcommands; each imports `<tool>_workflow` from `src.github.<tool>`
...
|------|-----|-------------|
| `cli.py` | 162 | Entry point — argparse dispatch for all 11 tools |
