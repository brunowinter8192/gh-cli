...
- **Entry point:** `cli.py` — argparse CLI, 13 subcommands (191 LOC)
...
- **REST tools:** 10 visible subcommands + `get_issue_comments` (internal-only) = 11 REST modules; all via `build_headers()` (`src/github/client.py`) — `GITHUB_API_BASE`, auth; per-tool `per_page` constants now local to each module (`SEARCH_REPOS_PER_PAGE=30` in `search_repos.py`, `SEARCH_CODE_PER_PAGE=30` in `search_code.py`); `repo_freshness` added as 10th visible REST subcommand
