...
- **Entry point:** `cli.py` — argparse CLI, 14 subcommands (199 LOC)
...
- **REST tools:** 12 visible subcommands + `get_issue_comments` (internal-only) = 13 REST modules; all via `build_headers()` (`src/github/client.py`) — `GITHUB_API_BASE`, auth; per-tool `per_page` constants now local to each module (`SEARCH_REPOS_PER_PAGE=30` in `search_repos.py`, `SEARCH_CODE_PER_PAGE=30` in `search_code.py`)
