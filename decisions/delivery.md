...
- **Entry point:** `cli.py` — argparse CLI, 18 subcommands (`search_repos` … `index_issues`)
...
- **Return contract:** all 18 `<tool>_workflow()` functions return `list[TextContent]`; `cli.py` line 272 `print(result[0].text)`
- **REST tools (15):** `build_headers()` (`src/github/client.py`) — `GITHUB_API_BASE`, auth, `RESULTS_PER_PAGE=20`
...
- `search_repos_workflow()` → `list[TextContent]` return annotation (representative; all 18 workflows carry this type, confirmed via `from mcp.types import TextContent` in each module)
