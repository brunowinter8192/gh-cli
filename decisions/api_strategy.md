...
- 15 modules use GitHub REST API v3 via `build_headers()` (`client.py`): 14 visible subcommands (search_repos, search_code, get_repo_tree, get_file_content, grep_file, grep_repo, index_issues, get_repo, list_releases, get_release, create_issue, update_issue, list_issues, get_issue) + `get_issue_comments` (internal-only helper of `index_issues.py`)
...
- `comment_issue` removed from surface; file deleted from `src/github/`
