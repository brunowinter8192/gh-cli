...
Topics are part of the Repositories API. No dedicated tool needed:
1. `get_repo(owner, repo)` → returns topics in output
2. `search_repos(query="topic:mcp-server")` → finds repos by topic

**`topic:` qualifier syntax for search_repos:**
- `topic:fastapi` — repos tagged with "fastapi"
- `topic:mcp-server language:python stars:>100` — combine with other qualifiers
- Multiple topics: `topic:machine-learning topic:python`
